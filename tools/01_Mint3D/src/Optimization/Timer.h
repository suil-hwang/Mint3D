...
namespace OptSolver {
// Template definition allowing custom clock types, defaulting to high-resolution clock.
...
class Timer {
   ...
   public:
    ...
    Timer() : is_stopped(false), is_paused(false), start_point(ClockType::now()) {}
    ...
    void pause() {
        if (!is_stopped && !is_paused) {   // Ensure the timer is running and not already paused.
    ...
    typename DurationType::rep elapsed() const {
        if (is_stopped) {
            ...
            return std::chrono::duration_cast<DurationType>(end_point - start_point).count();
        } else if (is_paused) {
            // Return the elapsed time until paused.
            return std::chrono::duration_cast<DurationType>(pause_point - start_point).count();
        } else {
            // Return the currently elapsed time if the timer is running.
            return std::chrono::duration_cast<DurationType>(ClockType::now() - start_point).count();
        }
    }
};
}   // namespace OptSolver
