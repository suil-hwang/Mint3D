...
Eigen::VectorXd MiNTEnergy::ComputeNormalizedMomentTensorsPerTet(const Eigen::MatrixXd& frames, int tet_id, int order,
    ...
    for (int i = 0; i < frame_per_tet; i++) {
        Eigen::VectorXd vec = frames.row(tet_id).segment(dim * i, dim).transpose();
        // avoid numerical instability from dividing by tiny vectors by just ignoring the fitting term
        // and letting smoothness solve for the directions for these vectors.
        // if (vec.norm() < 1e-4) {
        //     continue;
        // }
        Eigen::MatrixXd deriv_i;
...
double MiNTEnergy::ComputeIntegrabilityEnergyPerFace(const std::vector<Eigen::VectorXd>& tensors,
    ...
    if (hess) {
        // top_left_block *= weight;
        // bottom_right_block *= weight;
        if (is_PSD_proj) {
            //(*hess) = SelfAdjSPDProjection(*hess);
            hess->block<9, 9>(0, 0) += SelfAdjSPDProjection(top_left_block);
            hess->block<9, 9>(9, 9) += SelfAdjSPDProjection(bottom_right_block);
...
double MiNTEnergy::ComputeDivergenceEnergyByOrderWithTriplets(const Eigen::MatrixXd& frames, int ord,
    ...
    auto energy_func = [&](const Eigen::MatrixXd& input_frames, int face_id, Eigen::VectorXd* face_deriv,
        ...
        if (face_deriv) *face_deriv *= this->weights_.w_smooth;
        if (face_hess) *face_hess *= this->weights_.w_smooth;
        return energy *= this->weights_.w_smooth;
    };
...
double MiNTEnergy::ComputeSmoothnessEnergyWithTriplets(const Eigen::MatrixXd& frames, Eigen::VectorXd* deriv,
    ...
    for (int ord = 2; ord <= 6; ord += 2) {
        ...
        // energy += ComputeDivergenceEnergyByOrderWithTriplets(frames, ord, deriv ? &deriv_ord : nullptr,
        // hess_triplets,
        //                                                      is_PSD_proj, is_kruskal);
...
double MiNTEnergy::ComputeScaleFreeSmoothnessEnergyPerFace(const std::vector<Eigen::VectorXd>& tensors,
                                                           std::vector<Eigen::MatrixXd>* tensor_deriv,
                                                           std::vector<std::vector<Eigen::MatrixXd>>* tensor_hess,
    ...
    int tet_id1 = mesh_->faceTet(face_id, 1);
    if (tet_id0 == -1 || tet_id1 == -1) {
        return 0;   // boundary tet
    }

    double weight = dual_face_volumes_[face_id] / (dual_edge_lengths_[face_id] * dual_edge_lengths_[face_id]);

    Eigen::VectorXd tensor_diff = (tensors[tet_id0] - tensors[tet_id1]);
    double total_diff_squared = tensor_diff.squaredNorm();

    double energy = std::pow(total_diff_squared, p);
    ...
    if ((deriv || hess) && tensor_deriv) {
        ...
        if (deriv) {
            deriv->setZero();
            for (int i = 0; i < tensor_diff.size(); i++) {
                double grad_coeff = 2 * p * tensor_diff[i] * std::pow(total_diff_squared, p - 1);

                Eigen::VectorXd tensor_diff_deriv(18);
                tensor_diff_deriv.setZero();
                tensor_diff_deriv.segment<9>(0) = (*tensor_deriv)[tet_id0].row(i).transpose();
                tensor_diff_deriv.segment<9>(9) = -(*tensor_deriv)[tet_id1].row(i).transpose();

                (*deriv) += grad_coeff * tensor_diff_deriv;
            }
        }
        ...
        if (hess) {
            ...
            // First, compute cross-derivatives
            for (int i = 0; i < tensor_diff.size(); i++) {
                for (int j = 0; j < tensor_diff.size(); j++) {
                    Eigen::VectorXd tensor_diff_deriv_i(18), tensor_diff_deriv_j(18);
                    tensor_diff_deriv_i.setZero();
                    tensor_diff_deriv_j.setZero();

                    tensor_diff_deriv_i.segment<9>(0) = (*tensor_deriv)[tet_id0].row(i).transpose();
                    tensor_diff_deriv_i.segment<9>(9) = -(*tensor_deriv)[tet_id1].row(i).transpose();

                    tensor_diff_deriv_j.segment<9>(0) = (*tensor_deriv)[tet_id0].row(j).transpose();
                    tensor_diff_deriv_j.segment<9>(9) = -(*tensor_deriv)[tet_id1].row(j).transpose();

                    // Terms from symbolic derivatives
                    double cross_term1 = 2 * p * std::pow(total_diff_squared, p - 1);
                    double cross_term2 =
                        (2 * p * (p - 1)) * tensor_diff[i] * tensor_diff[j] * std::pow(total_diff_squared, p - 2);

                    Eigen::MatrixXd block_hess = (cross_term1 * (i == j ? 1.0 : 0.0) + cross_term2) *
                                                 tensor_diff_deriv_i * tensor_diff_deriv_j.transpose();

                    *hess += block_hess;
                }
            }

            // Add tensor derivative blocks
            for (int i = 0; i < tensor_diff.size(); i++) {
                double deriv_coeff = 2 * p * tensor_diff[i] * std::pow(total_diff_squared, p - 1);

                Eigen::MatrixXd top_left_block = deriv_coeff * (*tensor_hess)[tet_id0][i];
                Eigen::MatrixXd bottom_right_block = -deriv_coeff * (*tensor_hess)[tet_id1][i];
    ...
    // Apply weight
    energy *= weight;
    if (deriv) *deriv *= weight;
    if (hess) *hess *= weight;

    return energy;
}

// double MiNTEnergy::ComputeScaleFreeSmoothnessEnergyPerFace(
//     const std::vector<Eigen::VectorXd>& tensors,
//     std::vector<Eigen::MatrixXd>* tensor_deriv,
//     std::vector<std::vector<Eigen::MatrixXd>>* tensor_hess,
//     const Eigen::VectorXd& coeff_weights, int face_id, double p,
//     Eigen::VectorXd* deriv, Eigen::MatrixXd* hess, bool is_PSD_proj) {
...
//     for (int i = 0; i < tensor_diff.size(); i++) {
//         double x = tensor_diff[i];
//         double coeff = coeff_weights[i];

//         // Energy computation
//         double order = 1. / (cur_order - 1.);
//         double xi2 = x * x;
//         double xi2p = std::pow(xi2, order);
...
//         if ((deriv || hess) && tensor_deriv) {
//             double grad_coeff = 2 * coeff * order * std::pow(xi2, order - 1);
//             if (deriv) {
//                 (*deriv)[i] += grad_coeff * x;
//             }
//     //         Eigen::VectorXd tensor_diff_i_deriv(18);
//     //         tensor_diff_i_deriv.setZero();
//     //         tensor_diff_i_deriv.segment<9>(0) = (*tensor_deriv)[tet_id0].row(i).transpose();
...
double MiNTEnergy::ComputeSmoothnessEnergyByOrderWithTriplets(const Eigen::MatrixXd& frames, int ord,
    ...
    auto energy_func = [&](const Eigen::MatrixXd& input_frames, int face_id, Eigen::VectorXd* face_deriv,
        ...
        // double energy = ComputeScaleFreeSmoothnessEnergyPerFace(tensors, (face_deriv || face_hess) ? &tensor_deriv :
        // nullptr,
        //                                                face_hess ? &tensor_hess : nullptr, tensor_ord_weight_[ord],
        //                                                face_id, (1./(ord-1.)), face_deriv, face_hess,
        //                                                is_face_PSD_proj);
...
double MiNTEnergy::ComputeDeviationEnergyWithTriplets(const Eigen::MatrixXd& frames, Eigen::VectorXd* deriv,
    ...
    if (guided_tensor_coeffs_.empty() && this->weights_.b_fit) {
    ...
    // if (self_align_tensor_coeffs_.empty() && this->weights_.b_self_align) {
    //     std::cout << "guided frames are not provided" << std::endl;
...
double MiNTEnergy::ComputeDeviationEnergyPerTet(const Eigen::MatrixXd& frames, int tet_id, int order,
    ...
    // if (this->weights_.b_fit) {
    //     guided_tensor = guided_tensor_coeffs_[order][tet_id];
    // } else {
    //     guided_tensor = self_align_tensor_coeffs_[order][tet_id];
    // }
    ...
    double weight_target_align = base_weight * scale * (this->weights_.w_fit + scale * this->weights_.w_fit_precon);

    double lambda_penalty = this->weights_.lambda_penalty;
    double weight_self_align =
        base_weight * scale * (this->weights_.w_self_align * lambda_penalty + this->weights_.w_int_sym);   // + 1e0);

    base_weight = 0;
    if (this->weights_.b_fit) {
        base_weight += weight_target_align;
    }
    if (this->weights_.b_self_align) {
        base_weight += weight_self_align;
    }
...
double MiNTEnergy::ComputeUnitNormPenaltyPerTet(const Eigen::MatrixXd& frames, int tet_id, Eigen::VectorXd* deriv,
    ...
    for (int i = 0; i < 3; i++) {
        ...
        // This is for allowing the norm of the boundary normals to change.  Currently unused.
        // if (i == 0 && tet_id > mesh_not_extended_->nTets() && solve_params.boundary_hard_constraints ==
        // BoundaryHardConstraints::NoHardConstraints) {
        //     // weight = base_weight * (this->weights_.w_smooth * 10. * this->weights_.w_bound_rescale * 0.);   // +
        //     1e-8
        // } else
...
///////////// Viscosity Energy //////////////
// Compute the viscosity penalty. We assume that frames.rows() == mesh_->nTets() and frames.cols() == 9, that is
// 3 frames per tetrahedron
double MiNTEnergy::ComputeViscosityPenaltyWithTriplets(const Eigen::MatrixXd& frames, Eigen::VectorXd* deriv,
                                                       std::vector<Eigen::Triplet<double>>* hess_triplets,
                                                       bool is_PSD_proj) {
    std::vector<Eigen::Triplet<double>> T;
    ...
    auto energy_func = [&](const Eigen::MatrixXd& input_frames, int tet_id, Eigen::VectorXd* tet_deriv,
                           Eigen::MatrixXd* tet_hess, bool is_tet_PSD_proj) {
        return ComputeViscosityPenaltyPerTet(input_frames, tet_id, tet_deriv, tet_hess, is_tet_PSD_proj);
    };
    return AssembleEnergyFromTets(frames, deriv, hess_triplets, is_PSD_proj, energy_func);
}

// Compute the scaled jacobian penalty
double MiNTEnergy::ComputeViscosityPenalty(const Eigen::MatrixXd& frames, Eigen::VectorXd* deriv,
                                           Eigen::SparseMatrix<double>* hess, bool is_PSD_proj) {
    std::vector<Eigen::Triplet<double>> T;
    double energy = ComputeViscosityPenaltyWithTriplets(frames, deriv, hess ? &T : nullptr, is_PSD_proj);
...
///////////// Feature Alignment Energy //////////////
// Compute the viscosity penalty. We assume that frames.rows() == mesh_->nTets() and frames.cols() == 9, that is
// 3 frames per tetrahedron
double MiNTEnergy::ComputeFeatureAlignmentPenaltyWithTriplets(const Eigen::MatrixXd& frames, Eigen::VectorXd* deriv,
                                                              std::vector<Eigen::Triplet<double>>* hess_triplets,
                                                              bool is_PSD_proj) {
    // return AssembleEnergyFromTets(frames, deriv, hess_triplets, is_PSD_proj, energy_func);
    ...
    auto energy_func = [&](const Eigen::MatrixXd& input_frames, int tet_id, Eigen::VectorXd* tet_deriv,
                           Eigen::MatrixXd* tet_hess, bool is_tet_PSD_proj) {
        std::vector<Eigen::Vector3d> features = boundary_features_.at(tet_id - mesh_not_extended_->nTets());
        return ComputeFeatureAlignmentPenaltyPerTet(input_frames, tet_id, features, tet_deriv, tet_hess,
                                                    is_tet_PSD_proj);
    };
...
// Compute the scaled jacobian penalty
double MiNTEnergy::ComputeFeatureAlignmentPenalty(const Eigen::MatrixXd& frames, Eigen::VectorXd* deriv,
                                                  Eigen::SparseMatrix<double>* hess, bool is_PSD_proj) {
    std::vector<Eigen::Triplet<double>> T;
    double energy = ComputeFeatureAlignmentPenaltyWithTriplets(frames, deriv, hess ? &T : nullptr, is_PSD_proj);
...
double MiNTEnergy::ComputeViscosityPenaltyPerTet(const Eigen::MatrixXd& frames, int tet_id, Eigen::VectorXd* deriv,
                                                 Eigen::MatrixXd* hess, bool is_PSD_proj) {
    assert(frames.cols() == 9);
    double energy = 0;
    double base_weight = tet_volumes_[tet_id] / ave_edge_len_ / ave_edge_len_;
    ...
    weight = base_weight * scale * this->weights_.w_viscocity *
             lambda_penalty;   // weight = base_weight * 1e-13 * (1. / scale) * (1. / scale);

    // if (scale < 1e-6) {
    //     weight = base_weight * scale * this->weights_.w_viscocity *
    //              lambda_penalty;   // weight = base_weight * 1e-13 * (1. / scale) * (1. / scale);
    // } else {
    //     weight = base_weight * scale * this->weights_.w_int_sym * 1.e-8 * lambda_penalty;
    // }

    // if (scale < 1e-10) {
    //     weight = base_weight * scale * 7.5e-7 * lambda_penalty;
    // } else if (scale < 1e-7) {
    //     weight = base_weight * scale * 1e-7 * lambda_penalty;
    // } else if (scale < 1e-2) {
    //     weight = base_weight * scale * 1e-8 * lambda_penalty;
    //     // weight = base_weight * 1e-13 * (1. / scale) * (1. / scale);
    // } else {
    //     weight = base_weight * scale * 1e-9 * lambda_penalty;
    // }

    // if (scale < 1e-10) {
    //     weight = base_weight * scale * 7.5e-7 * lambda_penalty;
    // } else if (scale < 1e-2) {
    //     weight = base_weight * scale * this->weights_.w_viscocity * lambda_penalty;        // weight = base_weight *
    //     1e-13 * (1. / scale) * (1. / scale);
    // }
    // else {
    //     weight = base_weight * scale * this->weights_.w_viscocity * .1 * lambda_penalty;
    // }
    ...
    Eigen::VectorXd ft = frames_prev_inner_step_.row(tet_id);
    Eigen::VectorXd ft_outer = frames_prev_inner_step_.row(tet_id);

    // Eigen::VectorXd ft = frames_prev_outer_step_.row(tet_id);

    // double outer_weight = 1e-3 * 0.;
    // Energy calculation
    energy += weight * (f - ft).squaredNorm() * 0;
    // energy += weight * (f - ft_outer).squaredNorm() * outer_weight;

    if (deriv) {
        *deriv = 2 * weight * (f - ft) * 0;
        // *deriv = 2 * weight * (f - ft_outer) * outer_weight;
    }

    if (hess) {
        Eigen::Matrix<double, 9, 9> hess_energy = Eigen::Matrix<double, 9, 9>::Zero();

        hess_energy = 2 * weight * Eigen::Matrix<double, 9, 9>::Identity();
        // hess_energy += 2 * weight * Eigen::Matrix<double, 9, 9>::Identity() * outer_weight;
...
double MiNTEnergy::ComputeFeatureAlignmentPenaltyPerTet(const Eigen::MatrixXd& frames, int tet_id,
                                                        std::vector<Eigen::Vector3d>& tet_features,
                                                        Eigen::VectorXd* deriv, Eigen::MatrixXd* hess,
                                                        bool is_PSD_proj) {
    assert(frames.cols() == 9);
    // assert(mesh_not_extended_->nTets() <= tet_id);
    double energy = 0;
    double base_weight = tet_volumes_[tet_id] / ave_edge_len_ / ave_edge_len_;
    ...
    double constraint_weight = scale * lambda_penalty * this->weights_.w_int_sym;

    double weight;

    double unit_weight = 0;   // base_weight * scale * 1e-2;   // base_weight * scale * 1e3;

    // weight += base_weight * constraint_weight;
    ...
    ////
    // Adding a regularity term per sharp vertex does seem like it could be helpful, but not doing that in our
    // experiment.
    ////
    // if (nfeat == 2) {
    //     // weight *= 1e3;   // don't care so much about direction, but this will keep away from degenerate solution
    //     // unit_weight *= 1e6;   // should regularize away solutions which "smooth out" verts.
    //     // weight = base_weight * scale * this->weights_.w_feat_align;

    //     unit_weight = base_weight * scale * 1e3;

    //     // unit_weight += base_weight * constraint_weight;
    //     //
    // }

    int nfeat = tet_features.size();
    int nvoltets = mesh_not_extended_->nTets();

    for (int i = 1; i <= 2; i++) {
        Eigen::Vector3d feat;
        if (i <= nfeat && this->weights_.w_feat_align > 1e-6) {
            feat = tet_features[i - 1];
            weight = base_weight * scale * this->weights_.w_feat_align;

        } else {
            feat = boundary_normals_.row(tet_id - nvoltets);
            weight = base_weight * scale * 1e-6;
        }
        Eigen::Vector3d vec = frames.row(tet_id).segment<3>(3 * i);

        energy += weight * pow(feat.dot(vec), 2);
        // energy += unit_weight * std::pow(1 - vec.dot(vec), 2);

        if (deriv) {
            (*deriv).segment<3>(3 * i) += 2 * weight * feat.dot(vec) * feat;
            // (*deriv).segment<3>(3 * i) += 2 * unit_weight * (vec.dot(vec) - 1) * (2 * vec);
        }

        if (hess) {
            // Compute Hessian
            Eigen::Matrix3d outer = feat * feat.transpose();
            hess->block<3, 3>(3 * i, 3 * i) += 2 * weight * outer;
            // hess->block<3, 3>(3 * i, 3 * i) +=
            //     2 * unit_weight * (2 * Eigen::Matrix3d::Identity() * (vec.dot(vec) - 1) + 4 * vec * vec.transpose());
        }
    }
...
// Test function for computing deviation energy
void MiNTEnergy::TestComputeViscosityPenalty(const Eigen::MatrixXd& frames) {
    auto energy_func = [&](const Eigen::MatrixXd& input_frames, Eigen::VectorXd* deriv,
                           Eigen::SparseMatrix<double>* hess, bool is_PSD_proj) {
        return ComputeViscosityPenalty(input_frames, deriv, hess, is_PSD_proj);
    };
    std::cout << "\nTesting viscosity energy" << std::endl;
    TestGeneralFunction(frames, energy_func);
}

// Test function for computing deviation energy
void MiNTEnergy::TestComputeFeatureAlignmentPenalty(const Eigen::MatrixXd& frames) {
    auto energy_func = [&](const Eigen::MatrixXd& input_frames, Eigen::VectorXd* deriv,
                           Eigen::SparseMatrix<double>* hess, bool is_PSD_proj) {
        return ComputeFeatureAlignmentPenalty(input_frames, deriv, hess, is_PSD_proj);
    };
    std::cout << "\nTesting feature alignment energy" << std::endl;
    TestGeneralFunction(frames, energy_func);
}
