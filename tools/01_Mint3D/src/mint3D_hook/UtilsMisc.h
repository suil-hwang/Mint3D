...
void Svd2x2Helper(const Eigen::Matrix<T, 2, 2>& A) {
    ...
    std::cout << "vector " << c2 << " " << s2 << " sing value " << d2 << " sqrt " << std::sqrt(std::abs(d2))
              << std::endl;
}
...
static Eigen::Matrix4d rstar_from_r(Eigen::Matrix2d r) {
    Eigen::Matrix4d ret;
    double a = r(0, 0);
    double b = r(0, 1);
    double c = r(1, 0);
    double d = r(1, 1);
    ret << a * a, a * b, a * b, b * b, a * c, b * c, a * d, b * d, a * c, b * c, a * d, b * d, c * c, c * d, c * d,
        d * d;
...
static Eigen::Vector4d rstar_xcomp_from_r(Eigen::Matrix2d r) {
    Eigen::Vector4d ret;
    double a = r(0, 0);
    double b = r(0, 1);
    double c = r(1, 0);
...
//             // std::cout<<"the number of nonzeros "<<H_proj.nonZeros() << "number of non-zeros per dof " <<
//             H_proj.nonZeros() / (6*F.rows()) << " # rows " << H_proj.rows() << " faces " << F.rows() <<std::endl;
